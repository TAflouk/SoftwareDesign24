...
def instantiation_tests():
    ...
    try:
        for i in range(10):
            ball_bucket.append(Ball(r.randrange(1, 50)))
        ball_bucket[-1] = ball_bucket[-2]
...
def set_color_tests(ball_list):
    for b in ball_list:
        print(f"\tChanging col of ball {b}")
        b.set_color(rand_nums(30, 255, 3))  #  have you a func to gen a tup?
        print(f"\t\t\t  to {b}")
...
def logical_tests(ball_list):
    for op in logical_ops:
        ...
        for idx in range(len(ball_list)-1):
            try:
                result = logical_ops[op](ball_list[idx], ball_list[idx+1])
                print(f"\t{ball_list[idx]} {op}")
                print(f"\t{ball_list[idx+1]} = {result}")
            except:  # just leave without Warning or Error
                calc = f"{ball_list[idx].radius} {op} {ball_list[idx+1].radius}"
                print(f"\t***Could not do {calc}***\n") 
